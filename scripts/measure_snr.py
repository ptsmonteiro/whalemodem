...
So SNR/3kHz is measured in a single window (the received tone burst, from a
squelch-open reception) by comparing:
  - signal power in the fixed 0-3000 Hz passband, after subtracting estimated
    noise in that passband;
  - "noise" power: RMS in two side bands just outside the tone band
    (700-950 Hz and 2050-2300 Hz for that same profile),
    power-spectral-density-normalized and scaled to 3000 Hz. _bands_for_profile
    flags a noise band landing on a tone's 3rd
...
SNR/3kHz_dB = 10*log10(signal_power / estimated_noise_power_0_to_3000_hz)
...
def _estimate_snr_db(audio, sample_rate, signal_band, noise_bands):
    passband_rms = _band_rms(audio, sample_rate, 1.0, 3_000.0)
    ...
    noise_power_in_band = noise_psd * 3_000.0
    signal_power = max(0.0, passband_rms ** 2 - noise_power_in_band)

    if signal_power <= 0 or noise_power_in_band <= 0:
        return None, passband_rms, noise_power_in_band
    snr_db = 10 * np.log10(signal_power / noise_power_in_band)
    return snr_db, passband_rms, noise_power_in_band
...
def _measure_direction(tx: RadioTransport, rx: RadioTransport, tx_name, rx_name, profile):
    ...
    print(f"  signal+noise RMS (0-3000 Hz): {signal_rms:.6f}")
    print(f"  estimated noise power (0-3000 Hz): {noise_power_in_band:.3e}")
    if snr_db is None:
        print(f"  could not compute SNR (zero signal or noise) for {tx_name} -> {rx_name}")
        return None
    print(f"  SNR/3kHz {tx_name} -> {rx_name}: {snr_db:.1f} dB (confidence_threshold={profile.confidence_threshold})")
    return snr_db
