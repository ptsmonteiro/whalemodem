"""Controlled over-air runner for VF5 on the IC-705 / HT bench."""
    ...
    try:
        for trial in range(1, trials + 1):
            ...
            captured = audio_io.capture_while_transmitting(
                vf5.modulate(payload), tx_device, rx_device, ptt,
                samplerate=vf5.SAMPLE_RATE, pre_roll=0.4, post_roll=0.8,
                ptt_lead=0.22, ptt_tail=0.05)
            decoded = vf5.demodulate_debug(captured, payload)
            good = decoded.get("payload") == payload
            stem = f"vf5_{direction.replace('-to-', '_to_')}_t{trial:02d}"
            ...
            record = {
                ...
                "rs_block_corrections": decoded.get("rs_block_corrections"),
                "rs_max_block_corrections": decoded.get(
                    "rs_max_block_corrections"),
                ...
                "median_payload_evm_db": float(np.median(
                    decoded["symbol_evm_db"][vf5.HEADER_SYMBOLS:])),
...
def main():
    ...
    output = {
        ...
        "waveform": {
            "sample_rate": vf5.SAMPLE_RATE, "frame_samples": vf5.FRAME_SAMPLES,
            "frame_seconds": vf5.FRAME_SECONDS, "symbol_samples": vf5.SYMBOL_SAMPLES,
            "symbols": vf5.TOTAL_SYMBOLS, "carriers_hz": vf5.CARRIER_HZ.tolist(),
            "modulation": "pilot-assisted coherent square 16-QAM",
            "bits_per_carrier_symbol": 4,
            "payload_region_symbols": vf5.PAYLOAD_SYMBOLS,
            "pilot_positions": vf5.PILOT_POSITIONS.tolist(),
            "data_symbols": vf5.DATA_SYMBOLS,
            "payload_bits": vf5.PAYLOAD_BITS,
            "convolutional_packet_bytes": vf5.PACKET_BYTES,
            "outer_fec": (f"{vf5.RS_BLOCKS}x shortened "
                          f"RS({vf5.RS_CODEWORD_BYTES},{vf5.RS_DATA_BYTES})"),
            "rs_packet_bytes": vf5.RS_PACKET_BYTES,
            "outer_byte_interleaved": True,
            "user_payload_bytes": vf5.MAX_PAYLOAD_BYTES,
        },
