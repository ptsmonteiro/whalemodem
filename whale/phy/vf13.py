"""Combinatorial noncoherent MFSK for analog FM.

One M-ary decision per symbol, scored by noncoherent energy detection so
the receive path needs no phase reference anywhere -- unlike the 50 Hz OFDM
of `whale.phy.vf12`, which needs a fast comb-pilot channel tracker to
survive the phase drift this FM path has. `mapping` generalizes the one-hot
decision to a k-of-N combinatorial code: `active_tones` of `tone_count`
tones are on at once per symbol, chosen from a codebook built with the
combinatorial number system and (if C(N,k) is not a power of two)
truncated to the largest power-of-two codeword count. The demodulator
scores every codeword in that codebook directly via one matrix multiply --
exact max-log LLRs over the codebook actually used, not an approximation.

Everything downstream of the tone magnitudes is `whale.dsp`: Gray mapping,
`PacketCodec`, punctured K=7 convolutional coding and interleaving. The
preamble's head and sync symbols are PN draws from the same codebook, so
the whole frame has one level, crest factor and tone set.
...
class Vf13Waveform(waveform.ModeDescription):
    """A parametric MFSK waveform FAMILY BASE, not itself a shippable mode.

    Geometry, K subbands, framing and repetition are all fields; a rung
    (`Vf13Mode`) subclasses this and fixes `name` and `mode_id`, and
    `whale.modes.vf13.mode_for` builds other geometries by target frame
    duration for `scripts/sweep_mfsk_fm.py`. `family_base` marks the
    pattern for tests/test_layering.py; `name` and `mode_id` are sentinels
    here so the bare base cannot be mistaken for a mode.
    """
    ...
    def __post_init__(self):
        if not self.name or self.mode_id < 0:
            raise ValueError(
                "Vf13Waveform is a parametric waveform family base, not a "
                "shippable mode -- instantiate a rung (e.g. Vf13Mode) that "
                "sets its own name and mode_id")
        m = self.tone_count
        if m < 2 or m & (m - 1):
