"""Controlled over-air runner for VF3 on the IC-705 / HT bench."""
    ...
    try:
        for trial in range(1, trials + 1):
            ...
            captured = audio_io.capture_while_transmitting(
                vf3.modulate(payload), tx_device, rx_device, ptt,
                samplerate=vf3.SAMPLE_RATE, pre_roll=0.4, post_roll=0.8,
                ptt_lead=0.22, ptt_tail=0.05)
            decoded = vf3.demodulate_debug(captured, payload)
            good = decoded.get("payload") == payload
            stem = f"vf3_{direction.replace('-to-', '_to_')}_t{trial:02d}"
            ...
            record = {
                ...
                "median_payload_evm_db": float(np.median(
                    decoded["symbol_evm_db"][vf3.HEADER_SYMBOLS:])),
...
def main():
    ...
    output = {
        ...
        "waveform": {
            "sample_rate": vf3.SAMPLE_RATE, "frame_samples": vf3.FRAME_SAMPLES,
            "frame_seconds": vf3.FRAME_SECONDS, "symbol_samples": vf3.SYMBOL_SAMPLES,
            "symbols": vf3.TOTAL_SYMBOLS, "carriers_hz": vf3.CARRIER_HZ.tolist(),
            "payload_bits": vf3.PAYLOAD_BITS,
            "user_payload_bytes": vf3.MAX_PAYLOAD_BYTES,
        },
