...
Builds `whale-server`, `whale-configure` and `whale-test` into a standalone,
no-Python-required onedir bundle, so an end user does not need Python,
numpy/scipy, or a system hamlib/PortAudio install to run a station,
configure its radio inventory, or check it on air.
