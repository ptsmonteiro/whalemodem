"""Real-hardware trial runner for hf8's band-placement experiment.

This experiment does NOT introduce new PHY code: it reuses
experiments/hf6_multicarrier_v2/mc.py's CarrierSpec/MultiCarrierMode
verbatim (imported, never copied or modified -- hf6 stays untouched). The
only thing hf8 manipulates is WHERE the carrier(s) sit in the 300-2700 Hz
passband: a single carrier serves as the per-frequency SNR probe (N=1
MultiCarrierMode), and 2/3-carrier configs compare a placement clustered
into the empirically-best region against a wide-spread control at the same
carrier count. See RESULTS.md for the trial-by-trial evidence and the
placement hypothesis this is testing.

Run (from the repository root), e.g. a single-carrier SNR probe at 900 Hz:
    python experiments/hf8_band_placement_v4/hardware_test.py \
        --carrier-hz 900 --baud 300 --bps 1 --packet-bytes 16 --trials 3

Or a clustered 2-carrier config:
    python experiments/hf8_band_placement_v4/hardware_test.py \
        --carrier-hz 900 1500 --baud 600 --bps 3 --packet-bytes 16 --trials 5

Each trial: build one random payload per carrier, modulate the composite
signal, key the real TX radio once, capture once on the real RX radio,
demodulate each carrier independently, compare payload bytes and CRC per
carrier. No framing/ARQ layer -- same direct-PHY-probe pattern as
...
like every prior experiment in this series; there is no code path here
that ever calls the IC-705 transport's .send().
"""
...
# cross-correlate. Same fixed seed table as hf6, reused for continuity.
...
def main(argv=None, *, pair_factory=bench.radio_pair):
    ...
    ap.add_argument("--carrier-hz", type=float, nargs="+", default=[1500.0],
                     help="centre frequency (Hz) for each carrier; number of "
                          "values sets the carrier count (default: single-carrier probe)")
    ap.add_argument("--baud", type=float, default=300.0)
    ...
    output_dir = args.output_dir or Path("logs") / "mode_sweeps" / f"hf8_band_placement_v4-{stamp}"
    output_dir.mkdir(parents=True, exist_ok=True)

    print("hf8_band_placement_v4 smoke test (from-scratch, unregistered mode)")
