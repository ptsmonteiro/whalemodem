...
Total key-to-unkey (PTT ON -> PTT OFF) air time for one frame is
    ...
    output-stream startup + head_pad + frame body

and only "frame body" carries information. The head pad is disposable in-band
audio before it; output-stream startup is measured rather than configured.
...
PTT lead and head pad trade off before the sync word. Tail PN symbols have
been removed; ptt_tail remains only as an experimental carrier-only knob.

Pass/fail over 5 trials cannot resolve anything near a cliff edge. The decoder
also reports how many adjacent symbols of the head PN sequence it observed.
That turns a padded transmission into a measurement
...
Run one stage at a time, e.g.:
    ...
    python scripts/sweep_ptt_timing.py --points 0.22:0.08:0.05 --trials 10

Values are seconds. A --points entry is ptt_lead:head_pad:ptt_tail;
...
BASELINE = dict(ptt_lead=0.0, head_pad=framing.HEAD_PAD_SECONDS, ptt_tail=0.0)
...
def parse_point(text):
    lead, head, ptt_tail = (float(x) for x in text.split(":"))
    return dict(ptt_lead=lead, head_pad=head, ptt_tail=ptt_tail)
...
def main():
    ...
    ap.add_argument("--vary", choices=["ptt_lead", "head_pad", "ptt_tail"])
    ap.add_argument("--values", help="comma-separated seconds for --vary")
    ap.add_argument("--points", help="comma-separated ptt_lead:head_pad:ptt_tail")
