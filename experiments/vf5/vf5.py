"""VF5: VF4's 58-carrier OFDM geometry with square 16-QAM.

VF5 keeps the 48 kHz / 24 ms / 214-symbol frame, interleaved rate-1/2
convolutional code, shortened Reed-Solomon outer code, CRC32 and acquisition
strategy. Each carrier holds four bits in normalized square 16-QAM. Ten known
...
The 189 remaining data symbols provide a 43,848-bit coded payload grid.
"""
...
_GRAY_LEVELS = np.array([-3.0, -1.0, 3.0, 1.0]) / QAM16_SCALE
_QAM16_LABELS = np.unpackbits(
    np.arange(16, dtype=np.uint8)[:, None], axis=1)[:, -4:]
_QAM16_POINTS = np.array([
    _GRAY_LEVELS[i_index] + 1j * _GRAY_LEVELS[q_index]
    for i_index in range(4) for q_index in range(4)
])


def qam16_from_bits(bits: np.ndarray) -> np.ndarray:
    """Map quartets to normalized square 16-QAM points."""
    quartets = np.asarray(bits, dtype=np.uint8).reshape(-1, 4)
    indices = (8 * quartets[:, 0] + 4 * quartets[:, 1]
               + 2 * quartets[:, 2] + quartets[:, 3])
    return _QAM16_POINTS[indices]


def bits_from_qam16(values: np.ndarray) -> np.ndarray:
    """Hard-slice square 16-QAM values and return their four-bit labels."""
    values = np.asarray(values, dtype=np.complex128).reshape(-1)
    distances = np.abs(values[:, None] - _QAM16_POINTS[None, :]) ** 2
    return _QAM16_LABELS[np.argmin(distances, axis=1)].reshape(-1)


def _qam16_indices(values: np.ndarray) -> np.ndarray:
    values = np.asarray(values, dtype=np.complex128)
    return np.argmin(np.abs(values[..., None] - _QAM16_POINTS) ** 2, axis=-1)


# Shortened RS(249, 217) over GF(256), primitive polynomial 0x11d. Eleven
...
def qam16_scores(values: np.ndarray) -> np.ndarray:
    values = np.asarray(values, dtype=np.complex128)
    return -np.abs(values[..., None] - _QAM16_POINTS) ** 2


def qam16_hard_bits(values: np.ndarray) -> np.ndarray:
    return _QAM16_LABELS[np.argmax(qam16_scores(values), axis=-1)].reshape(-1)


def qam16_soft_bits(values: np.ndarray,
                    carrier_weights: np.ndarray) -> np.ndarray:
    """Coherent max-log bit reliabilities; positive means bit zero."""
    scores = qam16_scores(values)
    llrs = np.empty(np.asarray(values).shape + (4,), dtype=np.float64)
    for bit in range(4):
        llrs[..., bit] = (
            np.max(scores[..., _QAM16_LABELS[:, bit] == 0], axis=-1)
            - np.max(scores[..., _QAM16_LABELS[:, bit] == 1], axis=-1))
    return (llrs * np.asarray(carrier_weights)[None, :, None]).reshape(-1)
...
_PILOT_SIGN_BITS = _base._lfsr_bits(
...
def encode_payload_bits(payload: bytes) -> np.ndarray:
    ...
    if len(payload) > MAX_PAYLOAD_BYTES:
        raise ValueError(
            f"payload is {len(payload)} bytes; VF5 maximum is {MAX_PAYLOAD_BYTES}")
...
def demodulate(audio: np.ndarray) -> dict:
    ...
    hard_bits = qam16_hard_bits(data_values)
    payload_decisions = np.empty_like(payload_corrected)
    payload_decisions[PILOT_POSITIONS] = PILOT_VALUES
    data_decisions = _qam16_indices(data_values)
    payload_decisions[DATA_SYMBOL_INDICES] = _QAM16_POINTS[data_decisions]
    ...
    soft_bits = qam16_soft_bits(data_values, carrier_weights)
...
def describe() -> str:
    return (f"vf5: {N_CARRIERS}x square 16-QAM carriers {CARRIER_HZ[0]:.2f}-"
...
def _check_constants() -> None:
    ...
    assert FEC_INPUT_BITS == 21_924
    assert FRAME_SAMPLES == 249_600 and FRAME_SECONDS == 5.2
    assert PACKET_BYTES == 2_739 and UNUSED_INFO_BITS == 6
    assert RS_ENCODED_BYTES == 2_739 and RS_PACKET_BYTES == 2_387
    assert MAX_PAYLOAD_BYTES == 2_381
