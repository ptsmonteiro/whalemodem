...
from whale.scenario import (FM_SCENARIO_PRESETS, HF_SCENARIO_PRESETS,
                            FmScenario, HfScenario)


def test_hf_presets_build_all_stages_in_physical_order_and_expand_description():
    scenario = HfScenario.from_preset(
...
def test_all_scenario_names_and_overrides_are_available():
    assert set(HF_SCENARIO_PRESETS) == {"quiet", "moderate", "disturbed"}
...
def test_unknown_scenario_names_are_actionable():
    with pytest.raises(ValueError, match="unknown HF scenario preset"):
        HfScenario.from_preset(
