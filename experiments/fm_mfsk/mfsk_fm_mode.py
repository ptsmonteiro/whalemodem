"""FM-band parametric noncoherent MFSK: single wide bank or K parallel subbands.

Phase-1 design-space prototype for a 500-3000 Hz FM control/data waveform.
The channel notes that motivate it (see the phase-1 report, not retained
here per AGENTS.md) are: heavy carrier phase drift on the IC-705<->HT FM
path (a header estimate can walk 120 degrees across a frame) and a hard
~16 dB effective-SNR ceiling from signal-proportional self-noise (IMD/
limiter/handheld audio chain) that does not move with drive level and gets
worse the more tones are on the air at once. Both facts point the same way
as `experiments/hf16_mfsk_lowsnr`: noncoherent MFSK, because energy
detection needs no phase reference anywhere in the receive path.

This module generalizes that HF prototype in one way: `subbands` (K). With
K=1 it *is* HF16's geometry, transplanted to the FM passband -- one M-ary
one-hot decision per symbol. With K>1 the band is split into K disjoint
groups of `tone_count` (M) contiguous FFT bins sharing one symbol clock,
and each group makes its own independent one-hot decision every symbol, so
a symbol carries `K * log2(M)` bits instead of `log2(M)`. That is K tones
on the air at once instead of one -- more bandwidth efficiency, and (per
the self-noise assumption above) more IMD, so K is the deliberate knob
between rate and drive headroom. K=1 stays constant-envelope; K>1 sums K
cosines and buys speed by spending some of the peak-to-average headroom a
single-tone-at-a-time signal would otherwise keep.

Only subband 0 carries the preamble (sync and head patterns): acquisition,
refinement and the carrier-offset measurement are exactly
`whale.dsp.mfsk`'s existing correlate/refine/offset_hz, run once, unchanged
by K. The other K-1 subbands are silent until the payload.

Framing, FEC and the repetition/outer-interleave layer are unchanged from
HF16: `whale.dsp.PacketCodec` (length/CRC32/whitener/rate-1/2 convolutional
code, soft Viterbi) plus an optional `repeat` that sends every coded bit
`repeat` times at widely separated points in the frame.
"""
...
from whale import dsp, framing, rx_audio
...
#: Measured usable FM passband (see the phase-1 report); both band edges are
#: parameters below so a candidate can pull in from the steep cutoffs.
BAND_LO_HZ = 500.0
BAND_HI_HZ = 3000.0

#: Per-tone cosine coefficient with one subband on the air; scaled by
#: 1/sqrt(subbands) so summing K subbands keeps roughly the same total
#: transmitted power as one, rather than K times as much.
...
class FmMfskMode:
    ...
    name: str = "fm_mfsk"
    mode_id: int = MODE_ID
    symbol_samples: int = 480            # 100 Hz spacing at 48 kHz
    tone_count: int = 16                 # M tones per subband
    subbands: int = 1                    # K parallel one-hot groups
    mapping: str = "subband"             # "subband" | "combinatorial"
    active_tones: int = 4                # k tones on, combinatorial only
    ...
    fec_rate: str = "1/2"                 # "1/2","2/3","3/4","5/6","7/8"
    ...
    def describe(self) -> str:
        ...
        return (f"fm_mfsk M={self.tone_count} {tag} "
    ...
    def _soft_bits_combinatorial(self, metric: np.ndarray) -> np.ndarray:
        ...
        capped at 2**bits_per_symbol (<=4096 for the k<=6 candidates this
        prototype considers), not the full C(N,k).
        """
        masks = self.combination_masks
...
def mode_for(tone_count=16, *, subbands=1, frame_seconds=None,
    ...
    return FmMfskMode(tone_count=tone_count, subbands=subbands,
...
#: A ready instance for quick use (K=1, HF16-equivalent geometry on the FM band).
MODE = mode_for(tone_count=16, subbands=1, frame_seconds=5.0)
