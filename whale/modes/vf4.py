...
(Gray-labelled square 16-QAM) instead of VF6's 8.  Half the bits per symbol
buys back roughly the per-carrier SNR margin VF6 sacrifices to reach
256-QAM, so VF4 targets a clean 25 dB RF C/N path rather than VF6's excellent
35-40 dB path, while still comfortably beating VF3's 2-bit differential QPSK
on throughput.

    [128 cyclic prefix][1024 OFDM core] = 1152 samples

The 189 remaining data symbols provide a 43,848-bit payload grid, RS(254,238)
protected in 21 interleaved codewords.

This module is a self-contained fork of `whale/modes/vf6.py`'s pattern
(constellation order and RS sizing differ; acquisition, timing, pilot
tracking, and framing structure are unchanged) rather than a shared kernel,
matching how VF6 itself relates to VF3/VF5 in this codebase.
"""
...
UNUSED_GRID_BYTES = PACKET_BYTES - (PACKET_BYTES // 254) * 254
RS_BLOCKS = PACKET_BYTES // 254
...
# Square 16-QAM: 2 Gray-labelled bits per axis, 4 levels {-3,-1,1,3}/sqrt(10).
QAM16_SCALE = np.sqrt(10.0)
_GRAY_TO_LEVEL = np.empty(4, dtype=np.float64)
for _binary_index in range(4):
    _GRAY_TO_LEVEL[_binary_index ^ (_binary_index >> 1)] = (
        2 * _binary_index - 3) / QAM16_SCALE
_LEVEL_TO_GRAY = np.array([i ^ (i >> 1) for i in range(4)], dtype=np.uint8)


def qam16_from_bits(bits: np.ndarray) -> np.ndarray:
    """Map groups of 4 bits [i_hi, i_lo, q_hi, q_lo] to normalized 16-QAM."""
    groups = np.asarray(bits, dtype=np.uint8).reshape(-1, 4)
    i_label = (groups[:, 0] << 1) | groups[:, 1]
    q_label = (groups[:, 2] << 1) | groups[:, 3]
    return _GRAY_TO_LEVEL[i_label] + 1j * _GRAY_TO_LEVEL[q_label]


def _axis_gray(values: np.ndarray) -> np.ndarray:
    indices = np.clip(np.rint((np.asarray(values) * QAM16_SCALE + 3.0) / 2.0), 0, 3).astype(np.uint8)
    return _LEVEL_TO_GRAY[indices]


def bits_from_qam16(values: np.ndarray) -> np.ndarray:
    """Separable O(N) hard slicer for square 16-QAM."""
    values = np.asarray(values, dtype=np.complex128).reshape(-1)
    i_label = _axis_gray(values.real)
    q_label = _axis_gray(values.imag)
    bits = np.empty((len(values), 4), dtype=np.uint8)
    bits[:, 0] = (i_label >> 1) & 1
    bits[:, 1] = i_label & 1
    bits[:, 2] = (q_label >> 1) & 1
    bits[:, 3] = q_label & 1
...
def _qam16_decisions(values: np.ndarray) -> np.ndarray:
    bits = bits_from_qam16(values)
    return qam16_from_bits(bits).reshape(np.asarray(values).shape)


# Shortened RS(254, 238) over GF(256), primitive polynomial 0x11d.  The
# available payload grid (5,481 bytes) is not an exact multiple of the
# 254-byte codeword, so RS_BLOCKS codewords are byte-interleaved across the
# leading RS_ENCODED_BYTES of the grid and the remaining UNUSED_GRID_BYTES
# are left as zero padding, exactly as VF6 leaves its own remainder unused.
...
SYNC_VALUES = qam16_from_bits(_SYNC_BITS)
...
HEADER_VALUES = np.vstack((
    ...
    qam16_from_bits(_TRAINING_BITS).reshape(
...
def frame_constellation(payload: bytes) -> np.ndarray:
    ...
    payload_values[PILOT_POSITIONS] = PILOT_VALUES
    payload_values[DATA_SYMBOL_INDICES] = qam16_from_bits(
...
def demodulate(audio: np.ndarray) -> dict:
    ...
    estimated_intercept, estimated_slope, timing_confidence = _estimate_timing(
        analytic, start)
    # As in VF6: acquisition already anchors the FFT window inside the CP,
    # and this frame's target channel moves by well under one sample over
    # its duration, so the fitted timing is retained as a diagnostic rather
    # than perturbing the fixed FFT window.
    intercept = slope = 0.0
    ...
    hard_bits = bits_from_qam16(data_values)
    payload_decisions = np.empty_like(payload_corrected)
    payload_decisions[PILOT_POSITIONS] = PILOT_VALUES
    payload_decisions[DATA_SYMBOL_INDICES] = _qam16_decisions(data_values)
...
def demodulate_debug(audio: np.ndarray, reference_payload: bytes | None = None) -> dict:
    ...
    grid = errors.reshape(DATA_SYMBOLS, N_CARRIERS, 4)
...
def describe() -> str:
    return (f"vf4: {N_CARRIERS}x square 16-QAM carriers {CARRIER_HZ[0]:.2f}-"
...
def _check_constants() -> None:
    ...
    assert MAX_PAYLOAD_BYTES == 4_992
