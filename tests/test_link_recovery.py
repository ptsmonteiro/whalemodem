...
were fatal, and the ones that were not are the ones that hid the bug: a step
that leaves the peer transmitting at the registry's control mode is
survivable (the control mode is always a decode candidate), while a step
whose candidates collapse to (CONTROL,) alone is FATAL.
"""
...
def test_three_silent_attempts_downgrade_and_retry_the_same_chunk():
    ...
    try:
        ...
        assert a.tx_profile is VF14_4
        assert b.rx_profile is VF14_4
    finally:
        _stop(a, b)
...
def test_a_stale_connect_does_not_reset_a_live_session():
    ...
    try:
        ...
        own_supported = list(a.modes.supported_ids)
...
def test_mode_step_script_parses_the_environment_the_bench_uses():
    ...
    supported = modes.default_registry().supported_ids
    assert link._forced_mode_id({}) is None
    assert link._forced_mode_id({"WHALE_FORCE_MODE": str(VF13.mode_id)}, supported) == VF13.mode_id
    assert link._forced_mode_id({"WHALE_FORCE_MODE": "99"}, supported) is None
    # A forced mode_id can be falsy (e.g. 0) and still a real, deliberate
    # override, so "no override" has to be None rather than anything the
    # callers can confuse with a legitimate mode id.
    assert link._forced_mode_id({"WHALE_FORCE_MODE": str(VF14_4.mode_id)}, supported) == VF14_4.mode_id
    # A mode outside the default registry is honoured when the station
    # actually carries it.
    assert link._forced_mode_id({"WHALE_FORCE_MODE": "21"}, supported) is None
    assert link._forced_mode_id({"WHALE_FORCE_MODE": "21"}, tuple(supported) + (21,)) == 21
...
if __name__ == "__main__":
    ...
    test_three_silent_attempts_downgrade_and_retry_the_same_chunk()
