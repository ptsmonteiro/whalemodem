...
- `AwgnChannel`, seeded real white noise using SNR/3kHz;
...
C/N rather than SNR and is not comparable to SNR/3kHz audio AWGN
...
- `scripts/benchmark_simulated_channels.py` is the explicit Monte Carlo tool.
  It sweeps SNR/3kHz for AWGN/Watterson or RF C/N for complex FM, defaults
...
metadata. The project-wide channel convention is **SNR over a 3 kHz
passband**, written `SNR/3kHz`: mean waveform signal power divided by noise
power integrated from 0 through 3000 Hz. Channel metadata uses
`snr_3khz_db`; every reported number uses the same fixed bandwidth.
...
- `passband_3khz`: the canonical simulated-channel SNR. It is mean-square
  ...
  interval, divided by real-noise power integrated over 0--3000 Hz. If the
  interval is omitted, it is the complete waveform passed to the channel.
...
`carrier_snr_db` are waveform-specific quality estimates, not SNR/3kHz.
They are retained as decoder diagnostics for compatibility and must always be
qualified by their estimator name. Trial documents store them under
`decoder_metrics`, separately from canonical `channel_measurements`.
