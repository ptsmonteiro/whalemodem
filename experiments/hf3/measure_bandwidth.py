"""Reproducible statistical 99%-power occupied-bandwidth campaign for HF3.

The measurement uses the complete keyed waveform returned by ``hf3.modulate``
...
def run_campaign(*, trials: int, seed: int,
    ...
    for payload_len in payload_lengths:
        if not 0 <= payload_len <= hf3.MAX_PAYLOAD_BYTES:
        ...
        for trial in range(trials):
            ...
            measurement = occupied_bandwidth_99(
                hf3.modulate(payload), hf3.SAMPLE_RATE)
    ...
    return {
        "schema": "whalemodem.hf3_occupied_bandwidth.v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "method": {
            "signal": "complete keyed hf3.modulate waveform",
        ...
        "sample_rate_hz": hf3.SAMPLE_RATE,
        "seed": seed,
