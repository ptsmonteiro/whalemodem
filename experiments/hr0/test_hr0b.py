"""Focused checks for the experiment-local HR0-B redesign candidate."""

import numpy as np

from experiments.hr0 import benchmark, hr0b
...
def test_held_out_minus24_awgn_oracle_seed_decodes_checked_payload():
    ...
    result = _aligned(noisy, hr0b.FULL_CLASS)
    assert result["payload"] == payload
...
def test_real_receiver_wires_the_seven_ms_thirty_hz_preset_at_high_snr():
    ...
    result = hr0b.HR0B.decode(_capture(noisy))
    assert result["payload"] == payload
    assert result["candidate_rank"] <= hr0b.MAX_CANDIDATES
...
def test_invalid_inputs_and_wrong_class_remain_integrity_checked_and_bounded():
    for audio in (np.zeros(0), np.zeros((2, 2)), np.asarray([np.nan])):
        result = hr0b.HR0B.decode(audio)
        assert result["payload"] is None
        assert result["candidate_count"] <= hr0b.MAX_CANDIDATES

    tiny = hr0b.HR0B.encode(bytes(range(12)))
    wrong = _aligned(tiny, hr0b.FULL_CLASS)
