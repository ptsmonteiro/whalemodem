"""Robust noncoherent one-tone M-FSK for analog FM.
...
Each rung has short, medium and full payload grids sharing one preamble.
The short grid carries ACK/DISC/FLOOR traffic; the medium grid carries
typical CONNECT exchanges; the full grid carries DATA and long control
packets. CRC32 selects the decoded grid.
...
from whale import dsp, framing, rx_audio, waveform
...
class Vf14Waveform(waveform.ModeDescription):
    """A parametric one-tone M-FSK waveform FAMILY BASE.

    A rung fixes the tone count, the symbol length, the preamble and the
    three payload grids -- `whale.modes.vf14` declares three of them, and
    `whale.modes.fmht0` subclasses this to swap the codec. `family_base`
    marks the pattern for tests/test_layering.py.
    """
    ...
    #: same knob as `whale.phy.vf13.Vf13Waveform.preemph_db`.
