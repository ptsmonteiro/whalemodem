...
A parametric family: a rung fixes the constellation order, the code rate
and the frame budget, and inherits the 240-point FFT on the 50 Hz grid, the
comb-pilot channel tracker and the frame layout from here. Every symbol
carries `pilot_comb_stride`-spaced comb pilots whose per-symbol drift,
smoothed over `pilot_time_span` symbols, tracks the phase the FM path
rotates between the header and the end of the frame.
...
class Vf12Waveform(waveform.ModeDescription):
    """A parametric 50 Hz OFDM waveform FAMILY BASE, not a shippable mode.

    A rung (`Vf12Mode`, `Vf16Mode`, ...) subclasses this and fixes `name`,
    `mode_id` and the air format; `family_base` marks the pattern for
    tests/test_layering.py. `name` and `mode_id` are sentinels here so the
    bare base cannot be mistaken for a mode.
    """
    ...
    def __post_init__(self):
        if not self.name or self.mode_id < 0:
            raise ValueError(
                "Vf12Waveform is a parametric waveform family base, not a "
                "shippable mode -- instantiate a rung (e.g. Vf12Mode) that "
...
def mode_for(cls, **kwargs):
    """Build a rung of this family from sweep-style keyword arguments.

    Accepts the spellings `scripts/` sweeps use -- `bits_per_symbol` for the
    constellation order, and a `carrier_spacing_hz`/`carrier_offset_hz` that
    may only restate the fixed 50 Hz grid.
    """
